...
def get_video_token(interview_id):
    ...
    try:
        ...
        # Récupérer channel_id depuis Firestore si non fourni
        if not channel_id:
            channel_id = interview_data.get('channel_id')
            if not channel_id:
                return jsonify({'error': 'Aucun channel_id disponible'}), 400

        app_id = current_app.config['AGORA_APP_ID']
        app_certificate = current_app.config['AGORA_APP_CERTIFICATE']

        if not app_certificate:
            logger.warning("AGORA_APP_CERTIFICATE manquant → token désactivé")
            return jsonify({'error': 'Configuration Agora incomplète'}), 500

        # Générer un UID entier déterministe
        uid_int = generate_agora_uid(user_id)
